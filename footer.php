...
<div class="mobile-header-footer row">
    <a href="tel:79001234567" class="button-header-footer phone-footer-icon">
    ...
    <div class="modal-gallery col">
        ...
        <div class="modal-gallery-master row">
            ...
            <div class="modal-gallery-master-right row">
                <a class="modal-gallery-master-more" href="<?= the_permalink($master_id)?>" id="master-more">Подробнее о мастере</a>
                <div class="modal-gallery-master-button button-order open-order-modal"  data-master-id="<?= esc_attr($master_id); ?>">Записаться</div>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.open-order-modal').forEach(button => {
        button.addEventListener('click', () => {
            ...
            const option = document.querySelector(`.master-select .option[data-id="${masterId}"]`);
            if (option) {
                const name = option.querySelector('.option-name').innerText.trim();
                const image = option.querySelector('.option-avatar').style.backgroundImage
                    .replace('url(', '').replace(')', '').replace(/"/g, '');

                // Вызываем уже готовую функцию из твоего кода
                selectMaster(masterId, name, image);
            }
        });
    });
